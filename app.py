...
def getApplicationPool():
	...
	try :
		...
		print result
		collegeid = result[0]['collegeid']
		curs1.execute("SELECT current_application.studentid, major, act, sat, gpa  FROM current_application, students WHERE collegeid = %s AND acceptancestatus = 0 AND students.studentid = current_application.studentid", collegeid)
		result = []
		for row in curs1:
			obj = {
				'studentid' : row[0],
